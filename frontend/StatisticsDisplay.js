...
const getColor = (value) => {
  ...
  for (let i = 0; i < colorStops.length - 1; i++) {
    ...
    if (value >= start.value && value <= end.value) {
      ...
      const factor = (value - start.value) / range;
...
const StatBar = ({ label, value }) => {
  const animationProps = useSpring({
    from: { width: '0%', backgroundColor: getColor(0) },
    to: { width: `${value}%`, backgroundColor: getColor(value) },
...
const StatisticsDisplay = ({ selectedSkill, selectedJob }) => {
  ...
  useEffect(() => {
    if (selectedJob === 'All') {
      ...
      for (const job of Object.values(data)) {
        for (const [skill, value] of Object.entries(job[selectedSkill])) {
          combinedStats[skill] = (combinedStats[skill] || 0) + value;
        }
      }
      setStats(combinedStats);
    } else {
      setStats(data[selectedJob][selectedSkill] || {});
    }
  }, [selectedSkill, selectedJob]);
